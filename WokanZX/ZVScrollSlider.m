...
-(void)drawRect:(CGRect)rect
{
    ...
    CGFloat step = (_maxValue-_minValue)/6;
    ...
    for (int i = 0; i<=6; i++)
    {
        if (i% 6 == 0)
        {
            ...
            NSString *Num = [NSString stringWithFormat:@"%.f%@",(i*step+_minValue)/6,_unit];
...
-(void)drawRect:(CGRect)rect
{
    ...
    NSString *Num = [NSString stringWithFormat:@"%d%@",_maxValue/6,_unit];
    if ([Num floatValue]>1000000)
    {
        Num = [NSString stringWithFormat:@"%.f万%@",[Num floatValue]/10000.f,_unit];
    }
...
-(void)drawRect:(CGRect)rect
{
    ...
    NSString *Num = [NSString stringWithFormat:@"%d%@",_minValue/6,_unit];
    if ([Num floatValue]>1000000)
    {
        Num = [NSString stringWithFormat:@"%.f万%@",[Num floatValue]/10000.f,_unit];
    }
...
@property (nonatomic, assign) float              stepNum;
...
-(instancetype)initWithFrame:(CGRect)frame Title:(NSString *)title MinValue:(int)minValue MaxValue:(int)maxValue Step:(float)step Unit:(NSString *)unit HasDeleteBtn:(BOOL)hasDeleteBtn
{
    if(self = [super initWithFrame:frame])
    {
        ...
        _maxValue = maxValue;
        _step = step;
        _stepNum = (_maxValue-_minValue)/_step/6;
        _unit = unit;
        _scrollByHand = NO;
...
-(void)setRealValue:(float)realValue Animated:(BOOL)animated
{
    ...
    int time  = (int)(_realValue*_step);
    _valueTF.text = [NSString stringWithFormat:@"%d:%d0",time/6%24,time%6];
...
-(BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    {
        _valueTF.text = [NSString stringWithFormat:@"%d:%d0",_maxValue/6%24,_maxValue%6];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        if (!ruleView)
        {
            ruleView                 = [[ZVRulerView alloc]initWithFrame:CGRectMake(0, 0, dialGap*6, 50)];
            ruleView.backgroundColor = [UIColor whiteColor];
            ruleView.tag             = 9527;
            ruleView.unit            = _unit;
            [cell.contentView addSubview:ruleView];
        }
        ruleView.minValue = _step*6.f*(indexPath.item-1);
        ruleView.maxValue = _step*6.f*indexPath.item;
...
-(CGSize )collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        return CGSizeMake(dialGap*6.f, 50.f);
    }
}
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    if (_scrollByHand)
    {
        ...
        int val = value*_step;
        _valueTF.text = [NSString stringWithFormat:@"%d:%d0",val/6%24,val%6];
    }
}
